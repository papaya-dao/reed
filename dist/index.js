...
__export(src_exports, {
  ClarityContract: () => ClarityContract,
  ExtendedParsedTransactionResult: () => ExtendedParsedTransactionResult,
  ExtendedSimnet: () => ExtendedSimnet,
  TESTNET_ZERO_ADDRESS: () => TESTNET_ZERO_ADDRESS,
  isAddress: () => isAddress,
  range: () => range
});
...
var ExtendedSimnet = class _ExtendedSimnet {
  ...
  callReadOnlyFn(contract, method, args, sender = "") {
    ...
    if (this._debug) {
      ...
      console.log(`[DEBUG]     - Result: ${import_transactions.Cl.prettyPrint(parsedTransactionResult.result)}`);
    }
  ...
  callPublicFn(contract, method, args, sender = "") {
    ...
    if (this._debug) {
      ...
      console.log(`[DEBUG]     - Sender: ${_sender}`);
      console.log(`[DEBUG]     - Result: ${import_transactions.Cl.prettyPrint(parsedTransactionResult.result)}`);
      console.log(`[DEBUG]     - Events: [${parsedTransactionResult.events.map((ce) => JSON.stringify({ event: ce.event, data: ce.data.value ? import_transactions.Cl.prettyPrint(ce.data.value) : ce.data.value }, null, 4))}]`);
    }
    return new ExtendedParsedTransactionResult(parsedTransactionResult, this._expect);
  }
  getPrincipal() {
  ...
  getDeployedContractPrincipal(contractName) {
    return import_transactions.Cl.contractPrincipal(this.simnet.deployer, contractName);
  }
  _parseSender(sender) {
    if (!sender) {
...
var isAddress = (maybeAddress) => {
  ...
  try {
    (0, import_transactions2.createAddress)(maybeAddress);
    _isAddress = true;
  } catch (error) {
  }
  return _isAddress;
